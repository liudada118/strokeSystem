import React, { ReactComponentElement, useEffect, useImperativeHandle, useState } from "react";
...
echarts.use([GridComponent, LineChart, CanvasRenderer, PieChart, UniversalTransition, TooltipComponent, LegendComponent, BarChart, MarkLineComponent]);
...
export function Charts(props: circleData) {
  ...
  useEffect(() => {
    ...
    initCharts({ myChart: myChart, ydata: props.ydata, xdata: props.xdata, title: props.title, padding: props.padding });
    // myChart.resize();
    // myChart.resize();
    const resize = () => {
        ...
        <div className="yInfoContent">
          {
            yArr.map((a, index) => {
              return (
                <div className="yText">{a}{props.util ? props.util : 'h'}</div>
              )
            })
          }
        </div>
            ...
            {props.data.map((a, index) => {
              return (
                <div className="valueLineContent" style={{}}>
                  ...
                  <div className="valueLine" style={{ height: `${a * 100 / props.max}%`, backgroundColor: a > props.max * 0.7 ? '#04E4DC' : '#007CFD', position: 'relative' }}>
                    {a > 0 ? <div style={{ position: 'absolute', textAlign: 'center', transform: 'translate(-50%, -100%)', width: '3rem' }}>{a}{props.util ? props.util : 'h'}</div> : ''}
                  </div>
                </div>
              )
            })}
    ...
    <div className="yDate sleepTimeContent">
      ...
      <div className="sleepCharts">
        <div className="sleepChart">
          <SleepCharts ydata={props.data[0]} xdata={props.data[1]} zdata={props.data[2]} title={[]} index={2} /></div>
        <div className="sleepInfo">{
          ['BACK', 'LEFT', 'RIGHT'].map((a, i) => {
            return (
              <div style={{ display: 'flex', alignItems: 'center', marginBottom: i < 2 ? '0.68rem' : 'unset' }}> <div
                className="circle"
    ...
    const option = {
      ...
      xAxis: {
        ...
        axisLabel: {
          ...
          formatter: props.formatter ? function (value: any) {
            console.log(value)
      ...
      yAxis: {
        ...
        minInterval: props.moveFlag ? Math.ceil(Math.max(...props.ydata) / 10) * 10 / 5 : 1,
        axisLine: {
          show: true,
          color: '#B0B3B8',
          lineStyle: {
...
export function SleepCharts(props: circleData) {
  useEffect(() => {
    ...
    initCharts({ myChart: myChart, ydata: props.ydata, xdata: props.xdata, zdata: props.zdata });
    // myChart.resize();
    window.addEventListener("resize", function () {
      myChart.resize();
    });
        ...
        <div className="yDateInfoContent">
...

          {
            yArr.map((a: any, index) => {
              return (
                <div className="yText" style={{ position: 'relative', top: index == 2 ? '-0.7rem' : 'unset', bottom: index == 0 ? '-0.3rem' : 'unset' }}><span>{a}</span> </div>
              )
            })
          }
          <div></div>
                ...
                {newArr.map((b: any, index) => {
                  ...
                  return (
                    <div
                      ...
                      style={{ backgroundColor: colorArr[b[0]], width: `${b[1] * 100 / props.data.length}%`, height: '100%', position: 'relative', top: b[0] == 1 ? '-180%' : b[0] == 2 ? '180%' : 'unset', borderRadius: '2px' }}></div>
                  )
                })}
              </div>
            </div>
...
export function SleepSegAndProp(props: SleepSegAndPropType) {
  const [sleepNowNum, setSleepNowNum] = useState<any>({ index: -1, transform: 0 });
  const { data } = props
  ...
  function edgeProcess(index: number) {
    const showSleep = document.querySelector(`.showSleep${index}`) as HTMLElement
    const sleepStatuss = document.querySelector('.sleepStatuss') as HTMLElement
    const showSleepItem = document.querySelector('.showSleepItem') as HTMLElement
    ...
    let left = (showSleepLeft && sleepStatussLeft ? showSleepLeft - sleepStatussLeft - 50 < 0 ? sleepStatussLeft + 50 - showSleepLeft : 0 : 0)
    ...
    setSleepNowNum({ index: index, transform: left ? left : right ? -right : 0, left: left, right: right });
  }
