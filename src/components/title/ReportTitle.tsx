...
export default function ReportTitle() {
  ...
  const convertPath: (pathname: string) => convertPathReturn = (
    pathname: string
  ) => {
...
export const TitleLeftInfo = (props: any) => {
  ...
  return (
    <>
      {!props.isMobile ? (
        <PcLeftTitle text={props.text} path={props.path} />
      ) : (
        <div
          style={{ width: "100px" }}
          className="titleLeft"
...
export const PcLeftTitle = (props: any) => {
  ...
  const equipInfo = useSelector((state) =>
    selectEquipBySensorname(state, param.id)
  );
  ...
  return (
    <>
      {props.path == "report" ? (
        ...
        >
          <div
            style={{ paddingRight: "1.2rem", borderRight: "2px solid #BFBFBF" }}
          >
            工作台
          </div>
          <span className="w-[2px] mr-[15px]" />
          <span className="text-base font-semibold">
            {equipInfo?.patientName}
          </span>
        </div>
      ) : (
        <>{props.text}</>
...
export const TitleRightInfo = (props: any) => {
  ...
  if (props.path == "home") {
    ...
    return (
      <MessageRightTitle
        titleKey={titleKey}
        titleChangeGetMessage={titleChangeGetMessage}
      />
    );
    ...
    return (
      <div className="" style={{ display: "flex", justifyContent: "normal" }}>
        JQ HEALTHCARE
      </div>
    );
  }
