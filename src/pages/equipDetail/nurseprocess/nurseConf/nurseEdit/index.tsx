...
export default function NurseConfEdit(props: any) {
    ...
    const modalContentMap = {
        empty: {
            title: "删除项目",
            content: () => {
                return <div style={{ textAlign: "center", height: "4rem", lineHeight: "4rem" }}>
                    确认清空该护理计划？
                </div>
            },
            okFn: () => {
        ...
        save: {
            title: "应用护理计划",
            content: () => {
                return <div style={{ textAlign: "center", height: "4rem" }}>
        ...
        <>
            <div className="nurse_edit_box">
                <div className="nurse_edit_left">
                    ...
                    <div className="pc_use_nurse_form">
                        ...
                        <Button type="primary" htmlType="submit" style={{ width: '28rem', height: '2.5rem' }} onClick={onFinish} >
                            添加护理任务
                        </Button>
                    </div>
                </div>
                <div className="nurse_edit_right">
                    ...
                    <div className="bg-[#F5F8FA] flex items-center mx-[1rem] pl-[0.5rem]">
                        <img
                            ...
                            alt=""
                        />
                        <div className="text-[0.8rem] flex-1 text-[#929EAB] pr-[5px] py-[3px]">当前内容仅作为效果预览，不可作为实际页面使用</div>
                    </div>
                    <div className="right_nurse_list">
                        <NurseList list={nurseList} delTemp={delTemp} operType="add" extParams={{ className: 'modify' }} />
                    </div>
                    {
                        nurseList.length > 0 &&
                        <Button type="primary" htmlType="submit" style={{ width: 'calc(100% - 2rem)', margin: '0 1rem', height: '3rem' }} onClick={() => {
                            setConfirmType('save')
                            setConfirmSave(true)
                        }} >
                            保存护理计划
            ...
            {isShowChooseTemp && (
                <Modal
                    ...
                    footer={[
                        // 自定义页脚，只包含确认按钮
                        <Button key="ok" type="primary" style={{ padding: '0 1rem', height: "2rem", marginTop: "1.2rem" }} onClick={chooseTemp}>
                            选择此模版
                        </Button>,
                    ]}
                    width={"40.45rem"}
                >
                    <div className="default-popup">
                        <div className="w-full items-center ">
                            ...
                            <div className="flex w-[17rem] items-center mt-[0.4rem] bg-[#F5F8FA]">
                                <img
                                    className="w-[1rem] h-[1rem] mr-[0.5rem]"
                                    src={yuanquan}
                                    alt=""
                                />
                                <span className="text-[0.8rem]"> 特点：{defaultTempInfo.value}</span>
                            </div>
                        </div>
                        <NurseList list={tempList} extParams={{ isShowTime: true, className: 'preview' }}></NurseList>
                    </div>
                </Modal>
            )}

            <Modal title={modalContentMap[confirmType] && modalContentMap[confirmType].title} style={{ height: "4rem" }} okText='应用' cancelText='取消' visible open={confirmSave} onOk={() => {
