...
function alarmJudgeFn({ item, getFlag, alarm, onBedValue, alarmArrName, type }: alarmJudge) {
    // !  如果服务器请求到报警缓存
    ...
    // ! 报警开关数组包含该设备名 
    if (!(alarm && alarm[alarmArrName] && alarm[alarmArrName].includes(item.sensorName))) {
        return false
    }
...
export const alarmJudge = {
    ...
    onBedJudge: function ({ item, getFlag, alarm }: onBedType) {
        ...
        //     return false
        // }
        const flag = alarmJudgeFn({ item, getFlag, alarm, onBedValue: ALARMVALUE.onBed.value, alarmArrName: ALARMVALUE.onBed.alarmArrName, type: 'leave' })
    ...
    dropBedJudge: function ({ item, getFlag, alarm }: onBedType) {
        ...
        const flag = alarmJudgeFn({ item, getFlag, alarm, onBedValue: ALARMVALUE.dropBed.value, alarmArrName: ALARMVALUE.dropBed.alarmArrName, type: 'fallbed' })
    ...
    situpJudge: function ({ item, getFlag, alarm }: onBedType) {
        ...
        const flag = alarmJudgeFn({ item, getFlag, alarm, onBedValue: ALARMVALUE.situp.value, alarmArrName: ALARMVALUE.situp.alarmArrName, type: 'situp' })
...
export function neatEquips({ equipArr }: neatEquipsProps): neatReturn {
    ...
    * */
    ...
    Object.keys(ALARMTYPE).forEach((alarmtype) => {
        ...
        alarmSwitchTypeObj[ALARMTYPE[alarmtype as alarmtype].type] = ALARMTYPE[alarmtype as alarmtype].switchName
    ...
    return {
        newEquip, equip, riskArr, alarmSwitchTypeObj, cloudCatchAlarmArr
    }
}
...
export const findAlarmToCatch = ({ equips, cloudAlarmArr }: any) => {
    ...
    cloudAlarmArr.forEach((alarm: any) => {
        ...
        const obj = { ...equipInfo, outbedTime: Number(alarm.leavebed), situpTime: Number(alarm.situp), fallbedTime: Number(alarm.fallbed) }
        // console.log(obj)
...
export const findAlarmSwitch = ({ equip, cloudCatchAlarmArr }: findAlarmSwitchProps) => {
    // const cloudCatchAlarmArr: any = {}
...
export const returnRealAlarm = (obj: returnRealAlarmProps) => {
    ...
    let newSosArr: any = [], newSosSensornameArr: any = []
    catchArr.forEach((a: any, index: any) => {
        if (switchArr[alarmSwitchTypeObj[a.type]].includes(a.sensorName) && !newSosSensornameArr.includes(a.sensorName)) {
            newSosArr.push(a)
            newSosSensornameArr.push(a.sensorName)
            Object.keys(ALARMTYPE).forEach((alarmType) => {
                if (!riskArr[alarmType].includes(a.sensorName)) {
                    riskArr[alarmType] = riskArr[alarmType].filter((sensorName: any) => sensorName != a.sensorName)
                }
            })
        }
    })
...
export const changeOnerEquipInfo = ({ equips, changeInfo }: changeEquipParam) => {
    const res = [...equips]
